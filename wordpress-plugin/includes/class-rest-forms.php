...
class Form_Builder_REST_Forms {
    ...
    public static function register_routes() {
        ...
        register_rest_route(self::NAMESPACE, self::RESOURCE . '/(?P<id>\d+)', array(
            array(
                'methods' => 'GET',
        ...
        register_rest_route(self::NAMESPACE, '/forms/slug/(?P<slug>[a-zA-Z0-9_-]+)', array(
            array(
                'methods' => 'GET',
    ...
    public static function check_permission() {
        ...
        if (!isset($_REQUEST['_wpnonce'])) {
            return new WP_Error('missing_nonce', 'Nonce verification failed', array('status' => 403));
        }
        
        // Allow authenticated users or users with manage_options capability
    ...
    public static function get_forms($request) {
        $forms = Form_Builder_Database::get_all_forms();
    ...
    public static function get_form_by_slug($request) {
        ...
        $form = Form_Builder_Database::get_form_by_slug($slug);
    ...
    public static function create_form($request) {
        ...
        if (empty($body['title']) || empty($body['slug']) || empty($body['schema'])) {
            return new WP_Error('invalid_data', 'Missing required fields: title, slug, schema', array('status' => 400));
        }
        ...
        if (!is_array($schema)) {
            return new WP_Error('invalid_schema', 'Schema must be a valid object', array('status' => 400));
        }
        
        $form_id = Form_Builder_Database::create_form($title, $slug, $schema);
        
        if (!$form_id) {
            return new WP_Error('creation_failed', 'Failed to create form', array('status' => 500));
        }
    ...
    public static function update_form($request) {
        ...
        if (empty($body['title']) || empty($body['schema'])) {
            return new WP_Error('invalid_data', 'Missing required fields: title, schema', array('status' => 400));
        }
        ...
        $success = Form_Builder_Database::update_form($form_id, $title, $schema);
    ...
    public static function delete_form($request) {
        ...
        $success = Form_Builder_Database::delete_form($form_id);
